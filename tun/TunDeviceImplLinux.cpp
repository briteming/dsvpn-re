...
bool tun_setup(Context* context)
{
    ...
    boost::replace_first(ifUp, "$IF_NAME", context->TunIfName());

    boost::replace_first(ifUp2, "$IF_NAME", context->TunIfName());
...
{
    Shell shell;
    std::string ifDown = "iptables -t raw -D PREROUTING ! -i $IF_NAME -d $LOCAL_TUN_IP -m addrtype ! --src-type LOCAL -j DROP";
    boost::replace_first(ifDown, "$IF_NAME", context->TunIfName());
    boost::replace_first(ifDown, "$LOCAL_TUN_IP", context->LocalTunIP());
    shell.Run(ifDown);
    return true;
}
#endif