...
bool route_client_set_default(Context* context) {
    ...
    std::string add_default_gw_command = "ip route add default dev $IF_NAME table 42069 >>/dev/null 2>>/dev/null";
    std::string add_default_gw_command2 = "ip route add $EXT_IP via $EXT_GW_IP dev $EXT_IF_NAME >>/dev/null 2>>/dev/null";
    std::string add_default_gw_command4 = "ip rule add not fwmark 42069 table 42069 >>/dev/null 2>>/dev/null";
    std::string add_default_gw_command6 = "ip rule add table main suppress_prefixlength 0 >>/dev/null 2>>/dev/null";
    ...
    if (context->IPv6()) {
        std::string add_default_gw_command3 = "ip -6 route add default dev $IF_NAME table 42069 >>/dev/null 2>>/dev/null";
        std::string add_default_gw_command5 = "ip -6 rule add not fwmark 42069 table 42069 >>/dev/null 2>>/dev/null";
        std::string add_default_gw_command7 = "ip -6 rule add table main suppress_prefixlength 0 >>/dev/null 2>>/dev/null";
...
bool route_client_unset_default(Context* context) {
    std::string del_default_gw_command = "ip rule delete table 42069 >>/dev/null 2>>/dev/null";
    std::string del_default_gw_command2 = "ip route del $EXT_IP >>/dev/null 2>>/dev/null";
    std::string del_default_gw_command4 = "ip rule delete table main suppress_prefixlength 0 >>/dev/null 2>>/dev/null";


    boost::replace_first(del_default_gw_command2, "$EXT_IP", context->ServerIPResolved());

    Shell shell;
    shell.RunNoResult(del_default_gw_command);
    shell.RunNoResult(del_default_gw_command2);
    shell.RunNoResult(del_default_gw_command4);

    if (context->IPv6()) {
        std::string del_default_gw_command3 = "ip -6 rule delete table 42069 >>/dev/null 2>>/dev/null";
        std::string del_default_gw_command5 = "ip -6 rule delete table main suppress_prefixlength 0 >>/dev/null 2>>/dev/null";
